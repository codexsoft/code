    ...
    public static function executeUsingArgumentsArray(\Closure $callback, $vars = [], $owner = null)
    {
        ...
        foreach ((array) $reflectionParameters as $reflectionParameter) {
            ...
            if ($argumentName === 'vars') {
                ...
                array_push($callParameters, Arrays::valueOfKey($vars, $reflectionParameter->getName(), $defaultValue));
            }
        }

        return $owner ? $callback->call($owner, ...$callParameters) : $reflection->invokeArgs($callParameters);
